...
function generate() {
  ...
  postfix_artefact.postfix_expr = exp
    .infix_to_postfix(string.split(" "))
    .join(" ")
    .slice(0, -1);
  document.getElementById("questionid").innerHTML =
    "<span style='font-weight:bold;'>Question:</span> Evaluate the following postfix expression : " +
    postfix_artefact.postfix_expr;
  fun();
}
...
$("#push_button").on("click", function () {
  ...
  if ($.isNumeric(value)) {
    ...
    document.getElementById("numbers").value = "";
    document.getElementById("numbers").placeholder =
      "Operators are not yet pushed";
    document.getElementById("wrong").innerText =
...
$("#pop_button").on("click", function () {
  if (postfix_artefact.values.length) {
    var popped = postfix_artefact.values.pop();
    postfix_artefact.values2.push(popped);
    document.getElementById("output").innerHTML =
      "<span style='font-weight:bold;'>Popped Elements:</span> " +
      postfix_artefact.values2.join(" ");
    document.getElementById("correct").innerText =
...
$("#check_button").on("click", function () {
  ...
  var txtY = 200;
  if (
    eval_postfix(postfix_artefact.postfix_expr) == postfix_artefact.values[0] &&
    postfix_artefact.values.length == 1
  ) {
