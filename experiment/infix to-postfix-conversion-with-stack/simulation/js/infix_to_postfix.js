...
function clearCanvas() {
  postfix_artefact.ctx.clearRect(
    0,
    0,
...
function writeNumbers() {
  ...
  for (var i = 0; i < postfix_artefact.values1.length; i++) {
    ...
    var txtX =
      postfix_artefact.box_start_x +
      postfix_artefact.box_length / 2 -
      txtWidth / 2;
    var txtY =
      postfix_artefact.box_start_y -
      postfix_artefact.box_height * (i + 1) +
      postfix_artefact.box_height / 2 +
      8;
...
$("#push_tostack_button").on("click", function () {
  var inputValue = document.getElementById("pushtostack").value.trim();
  if (inputValue === "") {
    document.getElementById("wrong").innerText = "Please enter a value to push";
    return;
  }
  // Valid: multi-digit numbers, operators (+, -, *, /), and parentheses
  if (!/^(\d+|[+\-*/()]+)$/.test(inputValue)) {
    document.getElementById("wrong").innerText =
      "Invalid input. Only numbers, operators (+, -, *, /), and parentheses are allowed";
    return;
  }
  postfix_artefact.values1.push(inputValue);
  writeNumbers();
  clear();
});
...
function generate() {
  ...
  document.getElementById("questionid").innerHTML =
    "<span style='font-weight:bold;'>Question:</span> Convert the following expression to postfix: \n" +
    infix.join(" ");
...
$("#type_tooutput_button").on("click", function () {
  ...
  if (outputValue === "") {
    document.getElementById("wrong").innerText =
      "Please enter a value for output";
    return;
  }
  // Valid: multi-digit numbers and operators (+, -, *, /)
  if (!/^(\d+|[+\-*/]+)$/.test(outputValue)) {
    document.getElementById("wrong").innerText =
      "Invalid input. Only numbers and operators (+, -, *, /) are allowed in postfix output";
    return;
  }
...
$("#check_button").on("click", function () {
  ...
  var postfix = infix_instance
    .infix_to_postfix(infix.join("").split(""))
    .join("");
