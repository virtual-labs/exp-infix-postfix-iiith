...
Postfix_check.prototype.infix_to_postfix = function (infix1) {
  ...
  for (var i = 0; i < infix1.length; i++) {
    if ($.isNumeric(infix1[i])) {
      ...
      while (
        !Postfix_check.prototype.is_empty(stack) &&
        (Postfix_check.prototype.precedence(
          Postfix_check.prototype.top_stack(stack),
        ) > Postfix_check.prototype.precedence(infix1[i]) ||
          (Postfix_check.prototype.precedence(
            Postfix_check.prototype.top_stack(stack),
          ) === Postfix_check.prototype.precedence(infix1[i]) &&
            infix1[i] !== "^"))
